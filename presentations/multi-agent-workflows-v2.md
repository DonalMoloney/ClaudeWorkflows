...
"You've been watching orchestration happen without knowing it."
"The question is whether the plan lives in Claude's context — or in code."
This is the seed for the paradigm shift coming in Part 4.
SLIDE TRIM: "Every step consumed the same window. Every result accumulated. No isolation, no parallelism, no independent verification."
-->
...
<span class="kicker">The right jobs for this model</span>

<div class="diagram-grid cols-3" style="margin:14px 0">
  <div class="d-box orange">
    <span class="d-label">Repeatable tasks</span>
    <span class="d-sub">Define once, invoke on every PR. Same prompt, model, tools — every time.</span>
  </div>
  <div class="d-box blue">
    <span class="d-label">Context isolation</span>
    <span class="d-sub">Offload long reads. Your main session stays clean and focused.</span>
  </div>
  <div class="d-box green">
    <span class="d-label">Least-privilege</span>
    <span class="d-sub">A reviewer with no <code>Write</code> tool <em>cannot</em> edit files — by design.</span>
  </div>
</div>
...
EXAMPLES (say aloud): Code review for style guide → define once, invoke on every PR. Test generation → deterministic, auditable. Documentation on fixed templates → consistent across contributors.
-->
...
## Agent Teams — when work is truly collaborative

<!-- Speaker notes:
TRANSITION — Mark the shift clearly.
"Subagents are great when tasks are independent. Some work isn't."
"A feature spanning frontend and backend — those agents need to know what each other is doing."
-->
...
<span class="kicker">The feature that changes the quality ceiling</span>
...
<div class="d-flow" style="margin-top:14px">
  <div class="d-box orange">
    <span class="d-label">What this is</span>
    <span class="d-sub">The scientific method, implemented in JavaScript. Three independent critics attack every finding.</span>
  </div>
  <div class="d-box green">
    <span class="d-label">Why it matters</span>
    <span class="d-sub">No single-session AI can do this. It requires multiple independent contexts with no shared memory.</span>
  </div>
</div>

<!-- Speaker notes:
HIGH IMPACT — The moment audiences lean forward.
"/deep-research uses this exact pattern: fans out, cross-checks sources, votes internally."
"You're not asking Claude to check its own work. You're running three independent critics."
"The claim must survive a majority attack. Not plausible-sounding — adversarially tested."
-->
...
<span class="kicker">For discovery tasks with unknown scope</span>

```javascript
const bugs = [], seen = new Set()
let dry = 0

while (dry < 2) {                       // two consecutive empty rounds = exhausted
...
Runs until **two consecutive rounds find nothing new** — not just until a count is hit. Catches the long tail that `while (count < N)` always misses.

<!-- Speaker notes:
SUBTLE BUT CRITICAL — Easy to get wrong.
"The naive version stops at N. What if there are N+3 bugs?"
"Dedup against `seen`, not `confirmed` — rejected findings must still be tracked or they re-appear every round."
-->
